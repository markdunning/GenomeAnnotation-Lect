...
\item{cn.mops - Copy Number detection}
\item{exomeCopy - Copy Number from exome data}
...
ol <- findOverlaps(loc[1], trans)
ol
names(trans)[subjectHits(ol)]
@
...
\begin{frame}[fragile]
<<>>=
trans[subjectHits(ol)]
@

\end{frame}

\begin{frame}[fragile]{Lets check}

<<eval=FALSE>>=
cds <- cdsBy(txdb, "gene")
ol <- findOverlaps(cds[["79087"]], loc)
ol
cds[subjectHits(ol)]
@
